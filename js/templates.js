  ...
  container.innerHTML = `
    ...
    ${indexedFiltered.map(({ it, origIdx }) => {
      ...
      const meta = (qtyBadge || catBadge || storeChips)
        ...
        : '';
      const snapBadge = it.snapEligible ? '<span class="snap-badge snap-badge--row">SNAP</span>' : '';
...
export function openTplItemModal(idx, { buildCategoryOptions } = {}) {
  ...
  const snapCbTpl = document.getElementById('tpl-item-snap-eligible');
  if (snapCbTpl) snapCbTpl.checked = it ? !!it.snapEligible : false;
