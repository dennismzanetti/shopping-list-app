...
/**
 * Apply search / category / store filters and a sort to an item array.
 ...
 */
export function applyItemFilters(items, { search = '', category = '', store = '', sort = 'added' } = {}) {
  let result = [...items];
 ...
 * @param {Object}   current     - { search, category, store, sort }
 */
export function buildFilterToolbarHTML(prefix, categories, stores, current = {}) {
  const { search = '', category = '', store = '', sort = 'added' } = current;
  ...
  const hasFilter = search || category || store || sort !== 'added';
  return `
 ...
 * Calls onChange({ search, category, store, sort }) whenever any control changes.
 *
 * @param {string}   prefix
...
export function initFilterToolbar(prefix, onChange, initial = {}) {
  const state = { search: '', category: '', store: '', sort: 'added', ...initial };
  ...
  const onClear = () => {
    state.search = ''; state.category = ''; state.store = ''; state.sort = 'added';
